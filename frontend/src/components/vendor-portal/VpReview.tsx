...
export default function VpReview(props: VpReviewProps) {
  ...
  if (props.mode === 'consolidated') {
    ...
    const funcBreakdown = functions.map(func => {
      ...
      return {
        id: func.id,
        name: func.name_tr || func.name.replace(/\s*\(.*\)$/, ''),
    ...
    return (
      <ReviewLayout
        title="Inceleme ve Gonderim"
        subtitle="Degerlendirmenizi gondermeden once yanitlarinizi gozden gecirin."
      >
        {/* Stats */}
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <StatCard icon={<CheckCircle size={14} style={{ color: T.success }} />} label="Olgun (L3-L5)" count={maturityCounts.high} color={T.success} bg={T.successLight} border={T.successBorder} />
          <StatCard icon={<AlertTriangle size={14} style={{ color: T.warning }} />} label="Gelismekte (L2)" count={maturityCounts.medium} color={T.warning} bg={T.warningLight} border={T.warningBorder} />
          <StatCard icon={<XCircle size={14} style={{ color: T.danger }} />} label="Baslangic (L1)" count={maturityCounts.low} color={T.danger} bg={T.dangerLight} border={T.dangerBorder} />
          <StatCard icon={<HelpCircle size={14} style={{ color: T.textMuted }} />} label="Cevaplanmamis" count={maturityCounts.unanswered} color={T.textMuted} bg={T.bg} border={T.border} />
        </div>

        {/* Per-category answers */}
        <div style={{ ...card, padding: '18px 20px' }}>
          <h3 style={{ ...sectionHeader }}>Kategori Bazli Yanitlar</h3>
  ...
  return (
    ...
    >
      {/* Stats */}
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <StatCard icon={<CheckCircle size={14} style={{ color: T.success }} />} label="Uyumlu" count={stats.compliant} color={T.success} bg={T.successLight} border={T.successBorder} />
        <StatCard icon={<AlertTriangle size={14} style={{ color: T.warning }} />} label="Kismi" count={stats.partial} color={T.warning} bg={T.warningLight} border={T.warningBorder} />
        <StatCard icon={<XCircle size={14} style={{ color: T.danger }} />} label="Uyumsuz" count={stats.nonCompliant} color={T.danger} bg={T.dangerLight} border={T.dangerBorder} />
        <StatCard icon={<HelpCircle size={14} style={{ color: T.textMuted }} />} label="Cevaplanmamis" count={stats.unanswered} color={T.textMuted} bg={T.bg} border={T.border} />
      </div>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontFamily: T.fontSans, fontSize: 12, color: T.danger, margin: '4px 0 0', opacity: 0.85, lineHeight: 1.5 }}>
              Bu kontroller kurumsal guvenlik icin kritik oneme sahiptir. Gondermeden once yanitlamaniz onerilir.
            </p>
          </div>
        </div>
      )}
...
function FunctionBreakdownCard({ funcBreakdown, progressPct }: {
  ...
  return (
    <div style={{ ...card, padding: '18px 20px' }}>
      <h3 style={sectionHeader}>Fonksiyon Bazli Ilerleme</h3>
      ...
      <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <span style={{ fontFamily: T.fontSans, fontSize: 12, fontWeight: 600, color: T.textSecondary }}>Genel Ilerleme</span>
  ...
  return (
    <div style={{ padding: '14px 18px', borderRadius: 10, background: T.warningLight, border: `1px solid ${T.warningBorder}`, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontFamily: T.fontSans, fontSize: 13, fontWeight: 600, color: T.warning }}>{count} cevaplanmamis kontrolunuz var</span>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
        ...
        {moreCount > 0 && <span style={{ fontFamily: T.fontSans, fontSize: 11, color: T.warning, alignSelf: 'center' }}>+{moreCount} daha</span>}
      </div>
      <button onClick={onGoBack} style={{
        ...
        <ArrowLeft size={12} />Geri don ve tamamla
      </button>
    </div>
  );
}
...
function RespondentCard({ respondentName }: { respondentName: string }) {
  return (
    <div style={{ ...card, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div>
        <span style={{ fontFamily: T.fontSans, fontSize: 11, fontWeight: 600, color: T.textMuted, textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>Yanitlayan</span>
        <p style={{ fontFamily: T.fontSans, fontSize: 14, fontWeight: 600, color: T.textPrimary, margin: '2px 0 0' }}>{respondentName || 'Belirtilmemis'}</p>
      </div>
    </div>
  );
}
...
function ActionButtons({ submitting, onGoBack, onSubmit }: { submitting: boolean; onGoBack: () => void; onSubmit: () => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 8 }}>
      ...
      >
        {submitting ? (<><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} />Gonderiliyor...</>) : (<><Send size={16} />Degerlendirmeyi Gonder</>)}
      </button>
    </div>
  );
}
