...
export default function VendorEdit() {
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const data: UpdateVendorData = { name: form.name.trim() };
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ ...card, padding: 24 }} className="animate-pulse">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            {[1,2,3,4,5,6].map(i => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ height: 12, width: 80, borderRadius: 4, background: T.borderLight }} />
                <div style={{ height: 38, borderRadius: 8, background: T.borderLight }} />
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="animate-fade-in-up" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        {[
          ...
          { label: form.name || 'Detail', to: `/vendors/${id}` },
          { label: 'Edit', to: null },
        ].map((crumb, i, arr) => (
          <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {crumb.to ? (
              <Link to={crumb.to} style={{ fontFamily: T.fontSans, fontSize: 12, color: T.textMuted, textDecoration: 'none' }}
                onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.color = T.textSecondary; }}
                onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.color = T.textMuted; }}
              >{crumb.label}</Link>
            ) : (
              <span style={{ fontFamily: T.fontSans, fontSize: 12, color: T.textSecondary }}>{crumb.label}</span>
            )}
            {i < arr.length - 1 && <span style={{ fontFamily: T.fontSans, fontSize: 12, color: T.textMuted }}>/</span>}
          </span>
        ))}
      </div>
      ...
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <Link to={`/vendors/${id}`} style={{ textDecoration: 'none' }}>
          <div style={{
